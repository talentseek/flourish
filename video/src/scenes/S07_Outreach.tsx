...
const SUBJECT = "Exciting kiosk opportunity at Braehead Shopping Centre";
...
export const S07_Outreach: React.FC = () => {
  ...
  const panelY = interpolate(frame, [0, 15], [20, 0], {
    extrapolateRight: "clamp",
  });
  ...
  const subjectChars = Math.min(
    Math.floor(
      interpolate(frame, [20, 75], [0, SUBJECT.length], {
  ...
  // Sent badge
  const sentFrame = 140;
  const sentScale = interpolate(
    frame,
    [sentFrame, sentFrame + 8, sentFrame + 14],
    [0, 1.1, 1],
    { extrapolateRight: "clamp", extrapolateLeft: "clamp" }
  );
  const sentOpacity = interpolate(
    frame,
    [sentFrame, sentFrame + 5],
    [0, 1],
    { extrapolateRight: "clamp", extrapolateLeft: "clamp" }
  );
        ...
        >
          ...
          <div style={{ padding: "16px 28px", borderBottom: `1px solid ${BRAND.border}` }}>
            <div style={{ display: "flex", gap: 40, marginBottom: 8 }}>
              <div>
                <span
                  style={{ fontFamily: FONTS.mono, fontSize: 11, color: BRAND.textMuted }}
                >
                  FROM
                </span>
                <p
