#!/usr/bin/env tsx
/**
 * 🌐 ENHANCED WEBSITE DISCOVERY V2
 * Multiple fallback strategies: Google Places, Wikipedia, Bing, patterns
 */
...
const WEBSITE_PATTERNS = [
  ...
  (name: string) => `https://www.${name.toLowerCase().replace(/[^a-z0-9]/g, '-')}.co.uk`,
  (name: string) => `https://${name.toLowerCase().replace(/shopping\s+centre|shopping\s+center|retail\s+park/gi, '').replace(/\s+/g, '')}.co.uk`,
  (name: string) => `https://www.${name.toLowerCase().replace(/the\s+/gi, '').replace(/\s+/g, '')}.co.uk`,
];
...
async function googlePlacesSearch(name: string, city: string, postcode?: string): Promise<string | null> {
  try {
    ...
    const queries = [
      postcode ? `${name} ${postcode} UK` : `${name} ${city} UK shopping centre`,
      `${name} ${city} UK`,
      `${name} shopping centre ${city}`,
      `${name} retail park ${city}`,
    ];
...
async function checkWikipedia(name: string, city: string): Promise<string | null> {
  try {
    const wikiQuery = `${name} ${city} shopping centre`;
    const wikiUrl = `https://en.wikipedia.org/wiki/${encodeURIComponent(wikiQuery.replace(/\s+/g, '_'))}`;
    ...
    $('.infobox a[href^="http"]').each((_, elem) => {
    ...
    $('a.external[href^="http"]').each((_, elem) => {
      const href = $(elem).attr('href');
      const text = $(elem).text().toLowerCase();
      if (href && (text.includes('website') || text.includes('official'))) {
        try {
          const url = new URL(href);
          if (url.hostname.includes('.co.uk') || url.hostname.includes('.com')) {
            return href;
...
async function findWebsite(
  ...
  // Strategy 1: Google Places API (multiple queries)
  ...
  website = await checkWikipedia(name, city);
  if (website && await validateWebsite(website, name)) {
    return { website, method: 'Wikipedia' };
  }
...
async function main() {
  ...
  console.log('Multiple fallback strategies: Google Places, Wikipedia, Patterns\n');
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    const { website, method } = await findWebsite(loc.name, city, loc.postcode || undefined);
  ...
  console.log('\n' + '='.repeat(80));
  console.log('✅ ENHANCED WEBSITE DISCOVERY V2 COMPLETE!');
  console.log('='.repeat(80));
