#!/usr/bin/env tsx
/**
 * 📍 GOOGLE PLACES RE-ENRICHMENT
 * Re-run Google Places enrichment for locations that got websites after initial enrichment
 */
...
async function main() {
  console.log('📍 Google Places Re-Enrichment\n');
  console.log('📦 Enriching locations with websites but missing Google Places data\n');
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    const placeId = await searchPlace(loc.name, loc.city, loc.postcode || undefined);
    apiCalls++;
  ...
  console.log('\n✅ Google Places re-enrichment complete!');
  console.log(`\n📊 Results:`);
